...
We define two linear models based on the BOLASSO. The first model, called BOLASSO Logit, includes a second-stage ordinary logistic regression fit to only the features selected by the first-stage BOLASSO. Exhibit A7 provides the full regression output of the second stage of the BOLASSO Logit model. The coefficients in this second stage are not biased by regularization, as the LASSO coefficients in the first stage are. The second model, called BOLASSO Ensemble, is simply the averaged predicted probability across all 100 of the LASSO bootstrap replicates.
...
\textbf{Exhibit A6.} Features selected by BOLASSO as occurring with a non-zero coefficient in more than 90\% of LASSO bootstrap replicates.
...
\textbf{Exhibit A7.} Regression output for the second stage of the BOLASSO Logit model.
