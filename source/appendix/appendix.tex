...
Because the number of topics $t$ is not known \emph{a priori}, we tune this parameter by finding the $t$ with the best out-of-sample area under the operating-receiver characteristic curve (AUC) in a logistic regression that includes only the topic model features. We use only the training set for this tuning, and further subdivide it in half into topic training and topic validation sets. The tuning achieves AUCs on the topic validation set of 0.660 for 10 topics, 0.674 for 20 topics, 0.703 for 50 topics, 0.714 for 100 topics, 0.716 for 200 topics, and 0.696 for 500 topics. Therefore, we select the model with 200 topics as the final features.

\subsubsection{Low-Dosage Opioids}
...
As described in the main text, we perform a bootstrapped LASSO, or BOLASSO,\endnote{\refbach} on the summary features to consistently identify a model in the presence of correlated features. Exhibit A6 lists the summary features selected by the BOLASSO as occuring with a non-zero coefficient in more than 90 of the 100 LASSO bootstrap replicates.
...
We define two linear models based on the BOLASSO. The first model, called BOLASSO Logit, includes a second-stage ordinary logistic regression fit to only the features selected by the first-stage BOLASSO. Exhibit A6 provides the full regression output of the second stage of the BOLASSO Logit model. The coefficients in this second stage are not biased by regularization, as the LASSO coefficients in the first stage are. The second model, called BOLASSO Ensemble, is simply the averaged predicted probability across all 100 of the LASSO bootstrap replicates.
...
\textbf{Exhibit A5.} Low-dosage prescription opioids identified by the AHFS Pharmacologic/Therapeutic Classification category for opiate agonists.
...
\textbf{Exhibit A6.} Regression output for the BOLASSO Logit (a logistic regression of variables selected by BOLASSO as occurring with a non-zero coefficient in more than 90\% of LASSO bootstrap replicates).
...
\textbf{Exhibit A7.} Area under the receiver-operating characteristic curve (AUC) of neural network models using different subsets of administrative data and outcome definitions. Confidence intervals are calculated from 100 bootstrap replicates.
