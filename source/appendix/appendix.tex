...
As described in the main text, we perform a bootstrapped LASSO, or BOLASSO,\endnote{\refbach} to consistently identify a model in the presence of correlated variables. Exhibit A6 lists the variables selected by the BOLASSO as occuring with a non-zero coefficient in more than 90 of the 100 LASSO bootstrap replicates.
...
We define two linear models based on the BOLASSO. The first model, called BOLASSO Logit, includes a second-stage ordinary logistic regression fit to only the variables selected by the first-stage BOLASSO. Exhibit A6 provides the full regression output of the second stage of the BOLASSO Logit model. The coefficients in this second stage are not biased by regularization, as the LASSO coefficients in the first stage are. The second model, called BOLASSO Ensemble, is simply the averaged predicted probability across all 100 of the LASSO bootstrap replicates.
